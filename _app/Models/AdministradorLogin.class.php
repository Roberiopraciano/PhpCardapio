...
/**
 * AdministradorLogin.class [ MODEL ]
...
class AdministradorLogin {
    ...
    public function ExeLogin(array $UserData) {
        $this->Email = (string) strip_tags(trim($UserData['admin_email']));
        $this->Senha = (string) strip_tags(trim($UserData['admin_senha']));
        $this->setLogin();
    }
    ...
    public function CheckLogin() {
        if (empty($_SESSION['administrador']) || $_SESSION['administrador']['admin_level'] > $this->Level):
    ...
    private function setLogin() {
        if (!$this->Email || !$this->Senha || !Check::Email($this->Email)):
            $this->Error = ['Informe seu E-mail e senha para entrar!', WS_INFOR];
            $this->Result = false;
        elseif (!$this->getUser()):
            $this->Error = ['Os dados informados não são compatíveis!', WS_ALERT];
            $this->Result = false;
        elseif ($this->Result['admin_level'] > $this->Level):
    ...
    private function getUser() {
        ...
        $read = new Read;
        $read->ExeRead("ws_admin", "WHERE admin_email = :e AND admin_senha = :p", "e={$this->Email}&p={$this->Senha}");
    ...
    private function Execute() {
        ...
        $_SESSION['administrador'] = $this->Result;
